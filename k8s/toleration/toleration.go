...
func newPodTemplateSpec(podName string, imageName string, repository string, version string, pullPolicy corev1.PullPolicy, affinity *corev1.Affinity) (*corev1.PodTemplateSpec, error) {
...
func newBaseDeployment(deploymentName string, imageName string, namespace string, repository string, version string, pullPolicy corev1.PullPolicy, affinity *corev1.Affinity) (*appsv1.Deployment, error) {
	podTemplateSpec, err := newPodTemplateSpec(deploymentName, imageName, repository, version, pullPolicy, affinity)
	if err != nil {
		return nil, err
	}
...
func newPodAntiAffinity(key, topologyKey string, operator metav1.LabelSelectorOperator, values []string) *corev1.PodAntiAffinity {
...
func NewApiServerDeployment(namespace string, repository string, imagePrefix string, version string, pullPolicy corev1.PullPolicy, verbosity string) (*appsv1.Deployment, error) {
	affinity := &corev1.Affinity{
		NodeAffinity:    infraNodeAffinity(),
	...
	deployment, err := newBaseDeployment(deploymentName, imageName, namespace, repository, version, pullPolicy, affinity)
	if err != nil {
		return nil, err
	}
...
func NewControllerDeployment(namespace string, repository string, imagePrefix string, controllerVersion string, launcherVersion string, pullPolicy corev1.PullPolicy, verbosity string) (*appsv1.Deployment, error) {
	...
	imageName := fmt.Sprintf("%s%s", imagePrefix, deploymentName)
	deployment, err := newBaseDeployment(deploymentName, imageName, namespace, repository, controllerVersion, pullPolicy, affinity)
	if err != nil {
		return nil, err
	}
...
func infraToleration() []corev1.Toleration {
	...
	tolerations = append(tolerations, criticalAddonsToleration()...)
	return tolerations
}

func infraNodeAffinity() *corev1.NodeAffinity {
...
func main() {
	by, err := json.Marshal(infraToleration())
	if err != nil {
		panic(err)
	}
